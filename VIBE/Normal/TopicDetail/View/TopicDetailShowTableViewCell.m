//
//  TopicDetailShowTableViewCell.m
//  VIBE
//
...
@implementation TopicDetailShowTableViewCell
...
-(void)setTopicDetailShowTableCellWithInfo:(NSDictionary *)dict IsLastCell:(BOOL )isLast
{
    ...
    if ([type isEqualToString:@"text_Small"]) {
        ...
        [self setCellTextLabelWithFont:[VibeFont fontWithName:Default_Font_Middle size:16] Color:RGBA(73, 73, 73, 100) Text:content  LineSpace:6.0];
    }
...
-(void)setCellTextLabelWithFont:(UIFont *)font Color:(UIColor *)color Text:(NSString *)text LineSpace:(CGFloat )lineSpace
{
    ...
    [_detailInfoLabel setFrame:CGRectMake(20, 3, kScreenWidth -40, labelHeght +1)];
    [_detailInfoLabel setTextColor:color];
    
    [_backView setFrame:CGRectMake(5, 0, kScreenWidth -10, labelHeght +10 +_lastCellHeight)];
}
