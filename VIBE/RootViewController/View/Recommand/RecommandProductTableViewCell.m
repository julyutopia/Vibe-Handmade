//
//  RecommandProductTableViewCell.m
//  VIBE
//
...
@implementation RecommandProductTableViewCell
...
-(void)setProductCellWithModal:(VibeProductModal *)modal WithIndex:(NSInteger )index IsLastCell:(BOOL )last
{
    if (last == YES) {
        ...
        [_backView setFrame: CGRectMake(10, 0, _productViewWidth, _productViewHeight +10)];
...
{
    if ([_delegateee respondsToSelector:@selector(recommandProductCellClickWithIndex:)]) {
        [_delegateee recommandProductCellClickWithIndex:_productCellIndex];
    }
}
