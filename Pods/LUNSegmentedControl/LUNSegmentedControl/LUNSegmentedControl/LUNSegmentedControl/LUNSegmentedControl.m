...
- (void)setCornerRadius:(CGFloat)cornerRadius {
    ...
    if (self.applyCornerRadiusToSelectorView) {
        self.selectorView.layer.cornerRadius = 2;
    } else {
...
- (void)setCurrentState:(NSInteger)currentState {
    [self setCurrentState:currentState animated:YES];
}
...
- (UIBezierPath *)dribblePathForSelectorViewFromPercentage:(CGFloat)percent {
    ...
    CGPoint p1 = CGPointMake(height * 7/ 10, F(percent) * height);
    CGPoint p4 = CGPointMake(width - height* 7/ 10, F(1 - percent) * height);
    CGPoint p2;
    CGPoint p3;
